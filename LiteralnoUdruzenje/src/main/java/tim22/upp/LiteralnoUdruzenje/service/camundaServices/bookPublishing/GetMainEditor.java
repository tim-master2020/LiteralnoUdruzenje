...
@Service
public class GetMainEditor implements JavaDelegate {
    @Autowired
    IUserService userService;

    @Override
    public void execute(DelegateExecution delegateExecution) throws Exception {
        User lector = userService.findByRole(Role.MAINEDITOR).get(0);
        delegateExecution.setVariable("mainEditor", lector.getUsername());
    }
}
